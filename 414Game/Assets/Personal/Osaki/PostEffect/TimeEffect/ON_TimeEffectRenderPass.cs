...
{
    AfterOpaque,
    BeforeTransparamentObjects,
    BeforePostprocess,
    AfterPostprocess
}

public class ON_TimeEffectRenderPass : ScriptableRenderPass
{
    private const string RenderPassName = nameof(ON_TimeEffectRenderPass);
    ...
    private ON_TimeEffectVolume _volume;

    public ON_TimeEffectRenderPass(bool applyToSceneView, Shader shader)
    {
        if(shader == null)
        {
            return;
    ...
    public void Setup(RenderTargetIdentifier cameraColorTarget, PostprocessTiming timing)
    {
        ...
        _volume = volumeStack.GetComponent<ON_TimeEffectVolume>();
    }
    ...
    public static RenderPassEvent GetRenderPassEvent(PostprocessTiming postprocessTiming)
    {
        switch (postprocessTiming)
        {
            ...
            case PostprocessTiming.BeforeTransparamentObjects:
